...
class ElectronicSpider(scrapy.Spider):
	...
	def start_requests(self):
		...
		yield scrapy.Request(url=self.start_urls[0], headers=headers, callback=self.parse_category)
	...
	def parse_category(self, response):
		...
		for li in ul.find_all("li"):
			...
			if a:
				...
				header = ua.random
				headers = {"Accept": "*/*", "Accept-Encoding": "gzip, deflate", "User-Agent": header}
				yield scrapy.Request(link, headers=headers, callback=self.parse_data)
	...
	def parse_content(self, response):
		...
		try:
			...
			if "," in new_price1:
				new_price1 = new_price1.translate(None, ''.join(","))
			if "," in new_price2:
				new_price2 = new_price2.translate(None, ''.join(","))
